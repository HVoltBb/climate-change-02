...
Type objective_function<Type>::operator() ()
{
	...
	if(CppAD::Variable(month_k)){
		Type k = exp(month_k)/(1+exp(month_k))*2*PI;
		Type A = exp(month_mg);
		for(int i=0; i<12; i++){
			theta_l(i) = A*sin(2*PI*i/12.0 + k);
		}
	}else{
		theta_l = 0;
	}
	...
	for(int i=0; i < l1.size(); i++){
		...
		rg = reg_(reg(i));
			...
			if(td == 0){		
				...
				lt = grow::fish<Type>.growth_fn(lt, c + PAR5*effects + naoc(tb) + aoc(tb) + pnac(tb), k + (1-PAR5)*effects + naok(tb) + aok(tb) + pnak(tb), d2(i) - d1(i), PAR1, exp(lognu));
			} else {			
				// Released and recaptured in different months
				effects = indl(id) + theta_l(ms) + rg;
				lt = grow::fish<Type>.growth_fn(lt, c + PAR5*effects + naoc(tb) + aoc(tb) + pnac(tb), k + (1-PAR5)*effects + naok(tb) + aok(tb) + pnak(tb), 1 - d1(i), PAR1, exp(lognu));
				for(int j = 1; j < td; j++){
					effects = indl(id) + theta_l((ms+j)%12) + rg;
					lt = grow::fish<Type>.growth_fn(lt, c + PAR5*effects + naoc(tb+j) + aoc(tb+j) + pnac(tb+j), k + (1-PAR5)*effects + naok(tb+j) + aok(tb+j) + pnak(tb+j), days(tb+j), PAR1, exp(lognu));
				}
				// Second partial month
				effects = indl(id) + theta_l((ms+td)%12) + rg;
				lt = grow::fish<Type>.growth_fn(lt, c + PAR5*effects + naoc(t2(i)) + aoc(t2(i)) + pnac(t2(i)), k + (1-PAR5)*effects + naok(t2(i)) + aok(t2(i)) + pnak(t2(i)), d2(i), PAR1, exp(lognu));
			}
	...
	for(int i=0; i<Pdim; i++){
		gam_l(i) = grow::fish<Type>.growth_fn(ref_len, c + gam_c_n(i) + PAR5*effects, k + gam_k_n(i) + (1-PAR5)*effects, (Type) ref_tim/30.0, PAR1, exp(lognu));
	}
	...
	for(int i=0; i<Pdim; i++){
		gam_la(i) = grow::fish<Type>.growth_fn(ref_len, c + gam_c_a(i) + PAR5*effects, k + gam_k_a(i) + (1-PAR5)*effects, (Type) ref_tim/30.0, PAR1, exp(lognu));
	}
	...
	for(int i=0; i<Pdim; i++){
		gam_lp(i) = grow::fish<Type>.growth_fn(ref_len, c + gam_c_p(i) + PAR5*effects, k + gam_k_p(i) + (1-PAR5)*effects, (Type) ref_tim/30.0, PAR1, exp(lognu));
	}
	...
	for(int i = 0; i < days_p.size(); i++){
		...
		ref_p(i) = grow::fish<Type>.growth_fn(ref_len, c + gam_cn(i) + gam_ca(i) + gam_cp(i) + PAR5*effects, k + (1-PAR5)*effects, days_p(i), PAR1, exp(lognu));
	}
