...
obj = TMB::MakeADFun(
    data = list(l1 = as.numeric(datx$ReLenCM),
                ...
                PAR4 = 0,  # break point between daily resolution and monthly resolution [no effect here]
                PAR5 = 1,   # 1: linf, 0: k [location of intrinsic effects]
                PAR6 = 1   # 1: observation error,  
    ),
    parameters = list(c = c(298, 59, 47)[pickone],
                      logReg = 1,
                      reg_ = rep(0, 2),
...
fit_=nlminb(obj$par, obj$fn, obj$gr)
cat(TMBhelper::TMBAIC(fit_), '\n')

if(DEBUG){
fit_=nlminb(fit_$par, obj$fn, obj$gr)
